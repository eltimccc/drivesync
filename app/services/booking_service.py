...
class BookingService:
    ...
    def update_booking(booking: Booking, form_data: Dict, move_data: Dict) -> Dict:
        ...
        if not new_car_id or new_car_id == booking.car_id:
            ...
            return {
                "message": f"Бронирование ID {booking.id} обновлено",
                "new_booking": None,
            }
        ...
        if booking.status == "Аренда":
            ...
            if move_start and move_start != booking.start_date:  # Нужна пересадка
                BookingService._validate_transfer(
                    booking, move_start, move_end, new_car_id
                )
    ...
    @staticmethod
    def _should_create_transfer(
        booking: Booking, new_start: Optional[datetime]
    ) -> bool:
    ...
    def _parse_transfer_dates(move_data: Dict) -> Tuple[datetime, datetime]:
        ...
        try:
            move_start = (
                datetime.strptime(move_data["move_start"], "%d.%m.%Y %H:%M")
                if move_data.get("move_start")
                else None
            )
            move_end = (
                datetime.strptime(move_data["move_end"], "%d.%m.%Y %H:%M")
                if move_data.get("move_end")
                else None
            )
    ...
    def _should_create_transfer(
        ...
        if booking.status != "Аренда":
            raise ValueError(
                'Пересадка возможна только для броней со статусом "Аренда"'
            )
        return new_start is not None and new_start != booking.start_date

    @staticmethod
    def _validate_transfer(
        booking: Booking, move_start: datetime, move_end: datetime, new_car_id: int
    ) -> None:
        ...
        if move_end < move_start:
            raise ValueError(
                "Дата окончания пересадки не может быть раньше даты начала"
            )

        if move_start < booking.start_date:
            raise ValueError(
                "Дата начала пересадки не может быть раньше даты начала бронирования"
            )

        if not BookingService.is_car_available(
            new_car_id, move_start, move_end, booking.id
        ):
    ...
    def _change_car(booking: Booking, new_car_id: int) -> None:
        ...
        if not BookingService.is_car_available(
            new_car_id, booking.start_date, booking.end_date, booking.id
        ):
